...
import { DorsalRepository, EsfuerzoRepository, EventoRepository } from "../repositories/repositories.ts";
...
export class DorsalService {
    ...
    public async createDorsal(payload: Dorsal): Promise<boolean> {
        ...
        const eventosId = (await this.dorsalRepository.getAll({"usuario_id": { $eq: dorsal.usuario_id }}))
            .map(dorsal => dorsal.evento_id.toHexString());
        let eventosIncompletos = 0;
        
        if (eventosId.length > MAX_INSCRIPCIONES) {
            const eventos = (await this.eventoRepository.getEventos(eventosId)).entries();
            
    
            while (eventosIncompletos < 3) {
                const evento: Evento = eventos.next().value;
                if (evento) {
                    const dorsales = (await this.dorsalRepository.getAll({ "evento_id": { $eq: evento._id! } }))
                    ...
                    if (esfuerzosTotalesEnEvento < evento.objetivoKm) eventosIncompletos++;
                }
            }
        }

        const inscripcionesMinimas = (eventosId.length < MAX_INSCRIPCIONES) && (eventosIncompletos < MAX_INSCRIPCIONES);
